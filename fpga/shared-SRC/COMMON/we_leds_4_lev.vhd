-- $Id: we_leds_4_lev.vhd 1182 2026-02-08 09:26:38Z angelov $:

library ieee;
use ieee.std_logic_1164.all;

entity we_leds_4_lev is
generic(
...
port (
    ...
    -- code to be sent when intensity is 0 is all 0s (off)
...
architecture a of we_leds_4_lev is
...
-- lengths of the pulses in periods of the reference clock, in this case in 100 ns steps
-- begin of the sequence - reset pulse with this width (min 200 us according to datasheet!):
...
signal cnt_leds     : Integer range 0 to Nleds-1;
    ...
    -- 2 bits/LED: 0 - off, 1, 2, 3 - code_x_g|r|b
signal leds_in_sw   : std_logic_vector(2*Nleds-1 downto 0);
    -- blink flag, one bit/LED, 1 for blinking
signal leds_blink_sw : std_logic_vector(Nleds-1 downto 0);
...
begin
    ...
    begin
        for i in 0 to Nleds-1 loop
            if SWAP_ORDER then
                leds_in_sw(2*(Nleds-1-i)+1 downto 2*(Nleds-1-i) ) <= leds_in(2*i+1 downto 2*i);
                leds_blink_sw(Nleds-1-i)                          <= leds_blink(i);
            else
                leds_in_sw(2*i+1 downto 2*i) <= leds_in(2*i+1 downto 2*i);
                leds_blink_sw(i)             <= leds_blink(i);
            end if;
        end loop;
    end process;
    ...
    process(all)
    begin
        case leds_sr(leds_sr'high downto leds_sr'high-1) is
            when "00" => led_GRB <= (others => '0');
            when "01" => led_GRB <= code_1_g & code_1_r & code_1_b;
            when "10" => led_GRB <= code_2_g & code_2_r & code_2_b;
            when "11" => led_GRB <= code_3_g & code_3_r & code_3_b;
            when others => led_GRB <= (others => '-');
        end case;
    end process;
            ...
            case sm is
                ...
                when sm_reset =>
                    ...
                    if cnt_reset /= 0 then
                        ...
                        sm        <= sm_load_led;
                        for i in 0 to Nleds-1 loop
                            if leds_blink_sw(i)='1' and blink='0' then
                                leds_sr(2*i+1 downto 2*i) <= "00";
                            else
                                leds_sr(2*i+1 downto 2*i) <= leds_in_sw(2*i+1 downto 2*i);
                            end if;
                        end loop;
                    end if;
                ...
                when sm_send_led =>
                    if finish_GRB = '1' then
                        if cnt_leds /= 0 then
                            cnt_leds  <= cnt_leds - 1;
                            start_GRB <= '1';
                            leds_sr <= leds_sr(leds_sr'high-2 downto 0) & "00";
                            sm        <= sm_load_led;
                        else
                            sm <= sm_idle;
                            led_ic_in <= '0';
