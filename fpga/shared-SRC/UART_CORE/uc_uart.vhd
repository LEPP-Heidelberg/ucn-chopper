...
-- $Id: uc_uart.vhd 923 2022-08-18 06:30:25Z angelov $:
...
--  3   w   rx clear    write anything here to clear the rx-fifo (not ready),  bit 7 is clear tx fifo
--  3   r   status      Bits: 0 - send ready, 1 - send busy (no FIFO) or TX fifo full, 2 - recv ready, 3 - recv busy, 4 - TX_FIFO_BUSY
...
signal din_stored   : std_logic_vector(byte_size-1 downto 0);
signal din_ascii    : std_logic_vector(byte_size-1 downto 0);
signal din_tx       : std_logic_vector(byte_size-1 downto 0);
signal data_rx      : std_logic_vector(byte_size-1 downto 0);
signal din_tx_fifo  : std_logic_vector(byte_size-1 downto 0);
    ...
    begin
        if rising_edge(clk) then
            ...
            if sclr_tx_fifo='1' then
                ...
                case tx_sm is
                when tx_idle =>
                    if tx_fifo_empty='0' then -- some data in TX fifo
...
port map(
    ...
    status <= "000" & busy_f & busy_r & ready_r_l & busy_s & ready_s_l;

    process(clk)  -- latch the ready flags until the next write command
    begin
        if rising_edge(clk) then
            if rst_n = '0' or we = '1' then -- clear both flags if anything written
                ready_r_l <= '0';
                ready_s_l <= '0';
            else
