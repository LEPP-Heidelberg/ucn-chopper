...
for ((i = 1; i <= COUNT; i++)); do
    ...
    # Grab the ADC capture that --start armed for bridge 2 (channels 4-6:
    # voltage, I-shunt, I-prop; see C_hbr::read_adc_buffer()). Plot it with
    # ./plot_adc_buffer.py.
    ADC_FILE="${LOG_FILE%.*}_adc_run${i}.txt"
    "$HBR" --nll --get_adc_buff "$ADC_FILE"
    echo "ADC buffer: $ADC_FILE"
done
